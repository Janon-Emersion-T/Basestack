...
const login=await post('/api/auth/login',{email:'LIVE-AUTH@example.com',password});assert.equal(login.status,200);assert.equal(login.data.data.credentialsVerified,true);assert.equal(login.data.data.sessionIssued,true);
const sessionToken=login.data.data.session.token; secrets.push(sessionToken);
const current=await fetch(base+'/api/auth/me',{headers:{Authorization:`Bearer ${sessionToken}`}});assert.equal(current.status,200);
...
assert.equal((await fetch(base+'/api/auth/me',{headers:{Authorization:`Bearer ${sessionToken}`}})).status,401);
assert.equal((await post('/api/auth/password/reset',{token:reset,password})).status,400);
assert.equal((await post('/api/auth/login',{email:'live-auth@example.com',password})).status,401);
const renewed=await post('/api/auth/login',{email:'live-auth@example.com',password:replacement});assert.equal(renewed.status,200);
const logoutToken=renewed.data.data.session.token;secrets.push(logoutToken);
assert.equal((await fetch(base+'/api/auth/logout',{method:'POST',headers:{Authorization:`Bearer ${logoutToken}`}})).status,200);
assert.equal((await fetch(base+'/api/auth/me',{headers:{Authorization:`Bearer ${logoutToken}`}})).status,401);
